...
const BaseButton: FC<BaseButtonProps> = ({
  ...
  if (size === 'sm') {
    style += ' px-[clamp(10px,_3vw,_15px)] h-[clamp(30px,_0.909rem_+_1.7vw,_35px)] text-sm';
  } else if (size === 'lg') {
  ...
  if (isLink) {
    return (
      <Link
        href={href || '#'}
        className={cn(
          'flex h-[clamp(35px,_0.909rem_+_1.7vw,_40px)] w-fit items-center justify-center gap-2 rounded-full px-[clamp(20px,_3vw,_32px)] text-center text-[clamp(14px,_1.5vw,_16px)] font-medium shadow-none transition duration-300',
          style,
          className,
        )}
      >
